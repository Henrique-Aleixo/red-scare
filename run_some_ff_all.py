...
"""
run_some_ff_all.py
Run the some_ff.py solver on all graph instances in the data/ directory.
Updates the S column in results.txt with the results.
"""
import subprocess
import pathlib
import sys
...
DATA = ROOT / "data"
SOLVER = ROOT / "solvers" / "some_ff.py"
RESULTS_FILE = ROOT / "results.txt"
TIMEOUT = 60  # seconds per instance

def run_some_ff(instance_path):
    """Run some_ff.py on a single instance and return (success, result, error_msg)."""
...
def main():
    ...
    # Get all .txt files in data directory (excluding test files)
    instances = sorted([p for p in DATA.iterdir() if p.is_file() and p.suffix == ".txt" and not p.name.startswith("test_")])
    ...
    print(f"Running some_ff.py solver on all instances and updating results.txt...\n")
    print("=" * 80)
    ...
    for i, instance_path in enumerate(instances, 1):
        ...
        success, result, error = run_some_ff(instance_path)
        ...
        if success:
            ...
            if instance_name in instance_to_line:
                ...
                if len(data_lines[line_idx]) >= 7:
                    data_lines[line_idx][6] = result  # Update S column
                    updated_count += 1
                else:
