...
"""
run_some_all.py
Run the some.py solver on all graph instances in the data/ directory.
...
DATA = ROOT / "data"
SOLVER = ROOT / "solvers" / "some.py"
RESULTS_FILE = ROOT / "results.txt"
TIMEOUT = 60  # seconds per instance

def run_some(instance_path):
    """Run some.py on a single instance and return (success, result, error_msg)."""
...
def main():
    ...
    print(f"Running some.py solver on all instances and updating results.txt...\n")
    print("=" * 80)
    ...
    for i, instance_path in enumerate(instances, 1):
        ...
        success, result, error = run_some(instance_path)
