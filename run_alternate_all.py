#!/usr/bin/env python3
"""
run_alternate_all.py
Run the alternate.py solver on all graph instances in the data/ directory.
Updates the A column in results.txt with the results.
"""
import subprocess
import pathlib
import sys
...
DATA = ROOT / "data"
SOLVER = ROOT / "solvers" / "alternate.py"
RESULTS_FILE = ROOT / "results.txt"
TIMEOUT = 60  # seconds per instance

def run_alternate(instance_path):
    """Run alternate.py on a single instance and return (success, result, error_msg)."""
    ...
    try:
        ...
        # Parse output: first line should be "true" or "false"
        ...
        if answer not in ["true", "false"]:
            return False, None, f"Unexpected output: {answer} (expected 'true' or 'false')"
...
def main():
    ...
    print(f"Running alternate.py solver on all instances and updating results.txt...\n")
    print("=" * 80)
    ...
    for i, instance_path in enumerate(instances, 1):
        ...
        success, result, error = run_alternate(instance_path)
        ...
        if success:
            ...
            if instance_name in instance_to_line:
                ...
                if len(data_lines[line_idx]) >= 7:
                    data_lines[line_idx][2] = result  # Update A column (index 2)
                    updated_count += 1
                else:
