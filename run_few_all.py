...
"""
run_few_all.py
Run the few.py solver on all graph instances in the data/ directory.
Updates the F column in results.txt with the results.
"""
import subprocess
import pathlib
import sys
...
DATA = ROOT / "data"
SOLVER = ROOT / "solvers" / "few.py"
RESULTS_FILE = ROOT / "results.txt"
TIMEOUT = 60  # seconds per instance

def run_few(instance_path):
    """Run few.py on a single instance and return (success, result, error_msg)."""
    ...
    try:
        ...
        # Parse output: should be a number (minimum red vertices) or -1
...
def main():
    ...
    print(f"Running few.py solver on all instances and updating results.txt...\n")
    print("=" * 80)
    ...
    for i, instance_path in enumerate(instances, 1):
        ...
        success, result, error = run_few(instance_path)
        ...
        if success:
            ...
            if instance_name in instance_to_line:
                ...
                if len(data_lines[line_idx]) >= 7:
                    data_lines[line_idx][3] = result  # Update F column
                    updated_count += 1
                else:
